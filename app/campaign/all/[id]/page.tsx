...
export default function CampaignPage() {
  ...
  const handleStart = () => {
    ...
    const newForms = Array.from({ length: 1 }, () => ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!forms || forms.length === 0) {
        ...
        // คุณอาจจะแสดง alert หรือจัดการ error อื่นๆ ที่นี่
        Swal.fire("ข้อผิดพลาด", "ไม่พบข้อมูลฟอร์มที่จะส่ง", "error");
        return;
      }
      ...
      formData.append("name", firstForm.name);
      formData.append("birthdate", firstForm.birthdate);
      formData.append("birthmonth", firstForm.birthmonth);
      formData.append("birthyear", firstForm.birthyear);
      formData.append("birthtime", firstForm.birthtime);
      formData.append("birthconstellation", firstForm.birthconstellation);
      formData.append("age", firstForm.age);
      formData.append("wish", firstForm.wish);
        ...
        <div className="card w-full h-[70vh] flex items-center bg-white/90 rounded-lg">
          <div className="h-5/6 w-full p-4">
            <div className="text-center">
              <h1 className="text-2xl font-bold mb-4">กำหนดจำนวนเงิน</h1>
              <label
                htmlFor="formCount"
                className="block font-medium text-xl text-gray-700 mb-2"
              >
                จำนวนเงินที่ต้องการร่วมบุญ?
              </label>
              <input
                type="number"
                id="formCount"
